...
### Without WebGL (added 2026-09-05)

Steve's own Chrome refused a WebGL context (an extension or a graphics setting; the page cannot tell which), and the cards were empty boxes with a one-line apology. Now the build records a three-second MP4 of every explorer turning (Playwright drives a drag with the explorer's controls hidden; ffmpeg encodes 36 frames at 12 fps, 800×450, `libx264`), and each card carries that clip as a `<video>` with the product's `schema-map.svg` as its poster, `preload="none"` so viewers with WebGL never fetch it. When the WebGL probe fails the page adds `nogl` to the body, plays the clips, and shows a note that names the browser from the user agent and gives that browser's switch: `chrome://gpu` and the graphics-acceleration setting, `edge://gpu`, Firefox's `webgl.disabled`, Safari's Develop menu, and for all of them the private-window test that rules extensions in or out. A local build without ffmpeg falls back to the map image alone.
